...
class mongomarkov():
    ...
    def get_next(self, previous, current):  # get the next word based on the current and previous words
        return sorted(self.links.find({"prev": previous, "curr": current}),
                      key=lambda w: w["count"] * random.randint(0, 10000))[0]["next"]


    def get_previous(self, current, next):  # get the previous word based on the current and next words
        return \
            sorted(self.links.find({"next": next, "curr": current}),
                   key=lambda w: w["count"] * random.randint(0, 10000))[
                0]["prev"]

    def get(self, current):  # get the next and previous word based on the current word
        nodes = sorted(self.links.find({"curr": current}), key=lambda w: w["count"] * random.randint(0, 10000))
        if not nodes:
            raise Exception('"' + current + '" not in DB')
        else:
            node = nodes[0]
        previous = node["prev"]
        next = node["next"]
...
class markov:
    ...
    def talk(self, message):
        """generates nonsense based on what it's learnt, sorry if it says something rude!. usage: talk [about <thing>]"""
